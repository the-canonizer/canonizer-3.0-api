            ...
            <table width="100%" style=" background:#fff; padding:0px 30px;" cellpadding="0">
                ...
                <tr>
                    <td style="padding-top: 0px;  font-weight:400;">
                        <p> <a target="_blank"
                                href="{{ \App\Facades\Util::linkForEmail($data['nickname_url']) }}">{{ $data['nick_name']->nick_name }}</a>
                            has created the new thread
                            <a href="{{ \App\Facades\Util::linkForEmail($link) }}">{{ $data['thread_title'] }}</a></b>
                        </p> 
                    </td>
                </tr>
                <tr>
                        ...
                        @if (isset($data['subscriber']) && $data['subscriber'] == 1)
                            ...
                            <ul style="padding-left: 50px;">
                                @if (isset($data['support_list']) && count($data['support_list']) > 0)
                                    ...
                                    @endforeach
                                @else
                                    <li>You are subscribed to <b><a href="{{ \App\Facades\Util::linkForEmail($data['camp_url']) }}">
                        ...
                        @else
                            ...
                            <ul style="padding-left: 50px;">
                                @if (isset($data['support_list']) && count($data['support_list']) > 0)
                                    ...
                                    @endforeach
                                @else
                                    <li>You are directly supporting <b><a href="{{ \App\Facades\Util::linkForEmail($data['camp_url']) }}">
                                            {{ $data['camp_name'] }} </a></b></li>
                                @endif
