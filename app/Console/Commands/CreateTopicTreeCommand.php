...
class CreateTopicTreeCommand extends Command
{
    ...
    public function handle()
    {
        ...
        Log::info('Creating command history...');
        ...
        try {
            ...
            Log::error('tree:all command crashed: ' . $th->getMessage() . "\n" . $th->getTraceAsString());
            $commandHistory->error_output = $th->getMessage();
            $commandHistory->save();
        }
        ...
        if (count($topics)) {
            ...
            foreach ($topics as $value) {
                ...
                if ($value['topic_num'] < 166) {
                    Log::info('Upserting tree for topic: ' . $topic_num);
                    $tree =  TreeServiceFacade::upsertTree($topic_num, "blind_popularity", $asOfTime, $updateAll);
                    Log::info('Tree upserted for topic: ' . $topic_num);
                }
            }
        }
    }
