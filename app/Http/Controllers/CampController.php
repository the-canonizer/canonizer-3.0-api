...
class CampController extends Controller
{
        ...
        try {
            ...
            if ($camp) {
                ...
                try {
                    ...
                    PushNotification::pushNotificationToSupporter($request->user(),$request->topic_num, $camp->camp_num, $request->fcm_token, config('global.notification_type.Camp')) ;
                } catch (Throwable $e) {  
                    $data = null;
                    $status = 403;
    ...
    public function manageCamp(Request $request, Validate $validate)
    {
        ...
        try {
            $liveCamps = Camp::getAllLiveCampsInTopic($all['topic_num']);
            $nonLiveCamps = Camp::getAllNonLiveCampsInTopic($all['topic_num']);
            $camp_existsLive = 0;
            $camp_existsNL = 0;
            if (!empty($liveCamps)) {
