    ...
    public function store(Request $request, Validate $validate)
    {
        ...
        try {
            ...
            if ($thread) {
                ...
                $return_url =  config('global.APP_URL_FRONT_END') . '/forum/' . $request->topic_num . '-' . preg_replace('/[^A-Za-z0-9\-]/', '', $request->topic_name) . '/' . $request->camp_num.'-'.   preg_replace('/[^A-Za-z0-9\-]/', '', $request->camp_name) . '/threads';
    ...
    public function update(Request $request, Validate $validate, $id)
    {
        ...
        try {
            ...
            if(!$threads){
                ...
                $url = 'forum/' . $request->topic_num . '-' .  preg_replace('/[^A-Za-z0-9\-]/', '', $request->topic_name) . '/'  . $request->camp_num . '-' . preg_replace('/[^A-Za-z0-9\-]/', '', $request->camp_name) . '/threads';
                $activitLogData = [
                    'log_type' =>  "threads",
